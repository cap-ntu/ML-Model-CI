...
class CAdvisor(object):
    # Change default API information here
    ...
    def __init__(self, base_link=None, version=None, query_object=None):
        ...
        if base_link is None:
            self.base_link = self.DEFAULT_LINK
        else:
            self.base_link = base_link

        if version is None:
            self.version = self.DEFAULT_API_VERSION
        else:
            self.version = version

        if query_object is None:
            self.query_object = self.DEFAULT_OBJECT
        else:
    ...
    def request_all(self, t_object=None):
        ...
        Args:
            t_object (str): Request object for cAdvisor, can be 'docker' or 'machine'.
        """
    ...
    def request_without_cadvisor(self):
        """Return all the information from running containers expect cAdvisor's."""
    ...
    def request_by_name(self, name):
        """Return all the information by given specific name, according to API, name can be id or image name.

        Args:
            name (str): Filter, name can be container id or image name.
        """
    ...
    def request_by_id(self, id):
        """Return all the information by given specific container id.

        Args:
            id (str): Docker container's id, should be unique.
        """
    ...
    def request_by_image(self, image):
        """Return all the information by given specific container image name.

        Args:
            image (str): Docker container's image name.
        """
    ...
    def get_running_container(self, input_dict=None):
        """Return the basic information about running containers.

        Args:
            input_dict (dict): Extract information from the dict.
        """
    ...
    def get_stats(self, input_dict=None):
        """Return the stats information about running containers. Include metric like CPU, memory, etc.
    ...
    def get_specific_metrics(self, input_dict=None, metric=None):
        ...
        Args:
            input_dict (dict): Extract information from the dict.
            metric:  Metric field (diskio, cpu, diskio, memory, network, filesystem, task_stats, processes,
                accelerators).
        """
