...
class TRTConverter(object):
    ...
    def from_saved_model(
        ...
        # `~/.hysia/<model-name>/tensorflow-tfs/<version>/model.savedmodel`
    ...
    @staticmethod
    def build_model_inputs(node_def: Iterable[IOShape], remove_batch_dim=False):
    ...
    @staticmethod
    def build_model_outputs(node_def: Iterable[IOShape]):
        model_outputs = []
        for node in node_def:
    ...
    def generate_trt_config(
            ...
            instance_group: Iterable[ModelInstanceGroup] = None
    ):
        ...
        Arguments:
            save_path (Path): Model saving path name, generated by `hysia.hub.utils.generate_path`.
            inputs (Iterable[IOShape]): Input tensors shape definition.
            outputs: (Iterable[IOShape]): Output tensors shape definition.
            arch_name (str): Model architecture name。
            platform (TensorRTPlatform): TensorRT platform name.
            max_batch_size (int): Maximum batch size. Default to None, indicating the max batch size will be determined
            ...
            instance_group (Iterable[ModelInstanceGroup]): Model instance group (workers) definition. Default is to
        ...
        inputs = TRTConverter.build_model_inputs(inputs)
        outputs = TRTConverter.build_model_outputs(outputs)
    ...
    def _format_json(json_str: str):
        ...
        function_pipeline = [
            ...
            reformat_digit_list,
            reformat_json_key,
            partial(reformat_enum, keys=['data_type', 'kind']),
